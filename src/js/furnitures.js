...
import { openProductModal } from './furniture-detail.js'; // імпорт модалки
...
const imgArr = [
  { src: '../img/Furniture-img/1_1x-min.png', src2x: '../img/Furniture-img/1_2x-min.png', alt: 'Опис зображення' },
  { src: '../img/Furniture-img/2_1x-min.png', src2x: '../img/Furniture-img/2_2x-min.png', alt: 'Опис зображення' },
  { src: '../img/Furniture-img/3_1x-min.png', src2x: '../img/Furniture-img/3_2x-min.png', alt: 'Опис зображення' },
  { src: '../img/Furniture-img/4_1x-min.png', src2x: '../img/Furniture-img/4_2x-min.png', alt: 'Опис зображення' },
  { src: '../img/Furniture-img/5_1x-min.png', src2x: '../img/Furniture-img/5_2x-min.png', alt: 'Опис зображення' },
  { src: '../img/Furniture-img/6_1x-min.png', src2x: '../img/Furniture-img/6_2x-min.png', alt: 'Опис зображення' },
  { src: '../img/Furniture-img/7_1x-min.png', src2x: '../img/Furniture-img/7_2x-min.png', alt: 'Опис зображення' },
  { src: '../img/Furniture-img/8_1x-min.png', src2x: '../img/Furniture-img/8_2x-min.png', alt: 'Опис зображення' },
  { src: '../img/Furniture-img/9_1x-min.png', src2x: '../img/Furniture-img/9_2x-min.png', alt: 'Опис зображення' },
  { src: '../img/Furniture-img/10_1x-min.png', src2x: '../img/Furniture-img/10_2x-min.png', alt: 'Опис зображення' },
  { src: '../img/Furniture-img/11_1x-min.png', src2x: '../img/Furniture-img/11_2x-min.png', alt: 'Опис зображення' },
  { src: '../img/Furniture-img/12_1x-min.png', src2x: '../img/Furniture-img/12_2x-min.png', alt: 'Опис зображення' },
  { src: '../img/Furniture-img/13_1x-min.png', src2x: '../img/Furniture-img/13_2x-min.png', alt: 'Опис зображення' },
];
...
// Для збереження останніх завантажених меблів (для модалки)
...
function createMarkupCategory(categories, arrImg, activeId) {
  ...
  const firstLi = `
    <li class="category-item special-category wrapper_category ${activeId === '0' ? 'active' : ''}" data-id="0">
      <img class="img_category" src="${arrImg[0]?.src}" srcset="${arrImg[0]?.src2x} 2x" alt="${arrImg[0]?.alt}" />
  ...
  const otherItems = categories
    .map(
      ({ _id, name }, i) => `
      <li class="category-item wrapper_category ${activeId === _id ? 'active' : ''}" data-id="${_id}">
        <img class="img_category" src="${arrImg[i + 1]?.src}" srcset="${arrImg[i + 1]?.src2x} 2x" alt="${name}" />
...
function renderFurnitureList(furnitures, append = false) {
  if (!append) {
    lastLoadedFurnitures = furnitures;
    furnitureList.innerHTML = '';
  } else {
    lastLoadedFurnitures = [...lastLoadedFurnitures, ...furnitures];
  }

  if (!Array.isArray(furnitures) || furnitures.length === 0) {
    if (!append) furnitureList.innerHTML = '<p>Товари відсутні в цій категорії</p>';
    return;
  }
...
// Делегування для кнопки "Детальніше" — відкриваємо модалку з масиву lastLoadedFurnitures
...
async function init() {
  ...
  categoriesList.innerHTML = createMarkupCategory(categories, imgArr, currentCategoryId);
